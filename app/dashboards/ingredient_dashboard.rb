require "administrate/base_dashboard"

class IngredientDashboard < Administrate::BaseDashboard
  # ATTRIBUTE_TYPES
  # a hash that describes the type of each of the model's fields.
  #
  # Each different type represents an Administrate::Field object,
  # which determines how the attribute is displayed
  # on pages throughout the dashboard.
  ATTRIBUTE_TYPES = {
    category: Field::BelongsTo,
    id: Field::Number,
    name_Ingredient: Field::String,
    Price: Field::Number,
    Description: Field::Text,
    created_at: Field::DateTime,
    updated_at: Field::DateTime,
    image_file_name: Field::String,
    image_content_type: Field::String,
    image_file_size: Field::Number,
    image_updated_at: Field::DateTime,
    imageno_file_name: Field::String,
    imageno_content_type: Field::String,
    imageno_file_size: Field::Number,
    imageno_updated_at: Field::DateTime,
  }.freeze

  # COLLECTION_ATTRIBUTES
  # an array of attributes that will be displayed on the model's index page.
  #
  # By default, it's limited to four items to reduce clutter on index pages.
  # Feel free to add, remove, or rearrange items.
  COLLECTION_ATTRIBUTES = [
    :category,
    :id,
    :name_Ingredient,
    :Price,
  ].freeze

  # SHOW_PAGE_ATTRIBUTES
  # an array of attributes that will be displayed on the model's show page.
  SHOW_PAGE_ATTRIBUTES = [
    :category,
    :id,
    :name_Ingredient,
    :Price,
    :Description,
    :created_at,
    :updated_at,
    :image_file_name,
    :image_content_type,
    :image_file_size,
    :image_updated_at,
    :imageno_file_name,
    :imageno_content_type,
    :imageno_file_size,
    :imageno_updated_at,
  ].freeze

  # FORM_ATTRIBUTES
  # an array of attributes that will be displayed
  # on the model's form (`new` and `edit`) pages.
  FORM_ATTRIBUTES = [
    :category,
    :name_Ingredient,
    :Price,
    :Description,
    :image_file_name,
    :image_content_type,
    :image_file_size,
    #:image_updated_at,
    :imageno_file_name,
    :imageno_content_type,
    :imageno_file_size,
    #:imageno_updated_at,
  ].freeze

  # Overwrite this method to customize how ingredients are displayed
  # across all pages of the admin dashboard.
  #
  def display_resource(ingredient)
    "#{ingredient.name_Ingredient}"
  end
end
